...
def test_single_component_period_is_code_length():
    code_set = build_code_set([CodeComponent(name="CA", sequence=_code(1023))])
...
def test_colocated_components_share_period():
    ...
    code_set = build_code_set(
        [
            CodeComponent(name="I", sequence=_code(10230, 1)),
            CodeComponent(name="Q", sequence=_code(10230, 2)),
        ]
    )
...
def test_flat_packing_indexes_each_component_correctly():
    ...
    code_set = build_code_set([CodeComponent("A", a), CodeComponent("B", b)])
...
def test_index_of_and_metadata():
    code_set = build_code_set(
        [
            CodeComponent("D", _code(64, 1), power_weight=0.25),
            CodeComponent("P", _code(64, 2), power_weight=0.75),
        ]
    )
...
def test_overlay_and_subcarrier_flags():
    ...
    code_set = build_code_set(
        [
            CodeComponent("I", _code(32, 1), overlay=_code(10, 3)),
            CodeComponent("Q", _code(32, 2), subcarrier=boc),
        ]
    )
...
def test_rejects_float_sequences():
    ...
    with pytest.raises(ValueError, match="int8"):
        CodeComponent(name="I", sequence=np.zeros(10, dtype=np.float64))


def test_rejects_offset_outside_stride():
    with pytest.raises(ValueError, match="component_offset_chips"):
        CodeComponent(name="X", sequence=_code(8), chips_per_component_chip=2, component_offset_chips=2)


def test_rejects_duplicate_names():
    with pytest.raises(ValueError, match="unique"):
        build_code_set([CodeComponent("A", _code(8, 1)), CodeComponent("A", _code(8, 2))])
...
def test_rejects_uncovered_signal_chips():
    ...
    with pytest.raises(ValueError, match="claimed by no component"):
        build_code_set(
            [
                CodeComponent("A", _code(8, 1), chips_per_component_chip=2, component_offset_chips=0),
                CodeComponent("B", _code(8, 2), chips_per_component_chip=2, component_offset_chips=0),
            ]
        )
